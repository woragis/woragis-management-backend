...
func TestService_Register_Success(t *testing.T) {
	...
	mockRepo.On("createUser", mock.AnythingOfType("*management.User")).Return(nil).Run(func(args mock.Arguments) {
		user := args.Get(0).(*User)
		user.ID = userID
		createdUser = user
	})
	mockRepo.On("createProfile", mock.AnythingOfType("*management.Profile")).Return(nil)
	mockRepo.On("createSession", mock.AnythingOfType("*management.Session")).Return(nil)
